...
<html class="no-js h-100" lang="en">
  ...
  <div class="container-fluid">
        ...
        <aside class="main-sidebar col-12 col-md-3 col-lg-2 px-0">
          <div class="main-navbar">
            <nav class="navbar align-items-stretch navbar-light bg-white flex-md-nowrap border-bottom p-0">
              <a class="navbar-brand w-100 mr-0" href="<?php echo site_url('admin'); ?>" style="line-height: 27px;">
          ...
          <div class="main-content-container container-fluid px-4">
            ...
            <div class="row">
              <div class="col">
                <div class="card card-small mb-4">
                  <div class="card-header border-bottom" style="background-color: #99ff66;">
                    <h6 class="m-0">News yang telah dipublikasi (POST)</h6>
                  </div>
				...
				<?php for ($i = 1; $i <= count($this->session->userdata('columnName')); $i++) { 
					if ($this->session->userdata('columnName')[$i] == 'status_konten') continue;
				...
				<?php } ?>
                      ...
                      <tbody>
			<?php for ($i = 1; $i <= count($this->session->userdata('allNewsData')['id_konten']); $i++) { 								//iterasi untuk baris (x)
				$id = $this->session->userdata('allNewsData')['id_konten'][$i]; ?>
                        	<tr id="row<?php echo $id; ?>"> 
					<?php for ($k = 1; $k <= count($this->session->userdata('columnName')); $k++) {								//iteration for checking the data is it contained "post" or not
						$columnName = $this->session->userdata('columnName')[$k];		
						if ($this->session->userdata('allNewsData')[$columnName][$i] == 'post') {							//if it is containing "post", let the checking proses continued and "allNewsData" is ready to write into page
							for ($j = 1; $j <= count($this->session->userdata('columnName')); $j++) { 						//iterasi untuk kolom (y) pada baris (x) --> [x,y]
                        					$columnName = $this->session->userdata('columnName')[$j];
								if ($columnName == 'status_konten') continue; 									//--> to make sure data in "status_konten" column isn't echoed
								else ?><td><?php echo $this->session->userdata('allNewsData')[$columnName][$i]; ?></td> 
							<?php } ?>
							...
							<?php break;							
						}													//if not found, then end checking at this row and JUMP to the next row.
						if ($k == count($this->session->userdata('columnName'))) $i++;
					} ?>
				</tr>
			<?php } ?>
                      </tbody>
              ...
              <div class="col">
                <div class="card card-small overflow-hidden mb-4">
                  <div class="card-header" style="background-color: #ffff66;">
                    <h6 class="m-0 text-black">News yang belum dipublikasi (Draft)</h6>
                  </div>
			...
			<?php for ($i = 1; $i <= count($this->session->userdata('allNewsData')['id_konten']); $i++) { 								//iterasi untuk baris (x)
				$id = $this->session->userdata('allNewsData')['id_konten'][$i]; ?>
					...
					<?php for ($k = 1; $k <= count($this->session->userdata('columnName')); $k++) {								//iteration for checking the data is it contained "post" or not
						$columnName = $this->session->userdata('columnName')[$k];		
						if ($this->session->userdata('allNewsData')[$columnName][$i] == 'draft') {							//if it is containing "post", let the checking proses continued and "allNewsData" is ready to write into page
							for ($j = 1; $j <= count($this->session->userdata('columnName')); $j++) { 						//iterasi untuk kolom (y) pada baris (x) --> [x,y]
                        					$columnName = $this->session->userdata('columnName')[$j];
								if ($columnName == 'status_konten') continue; 									//--> to make sure data in "status_konten" column isn't echoed
								else ?><td><?php echo $this->session->userdata('allNewsData')[$columnName][$i]; ?></td> 
	...
	<script>
		function reassuringDeleteForPostNews(id) {
    			document.getElementById("row"+id).style.color = "white";
    			document.getElementById("row"+id).style.backgroundColor = "red";
			setTimeout(popUp, 10);
			function popUp() {
				if (confirm("Apakah anda yakin ingin menghapus News dengan ID="+id+"?")) {
					alert("News dengan ID="+id+" telah berhasil dihapus!");
		...
		function reassuringDeleteForDraftNews(id) {
    			document.getElementById("row"+id).style.color = "white";
    			document.getElementById("row"+id).style.backgroundColor = "red";
			setTimeout(popUp, 10);
			function popUp() {
				if (confirm("Apakah anda yakin ingin menghapus News dengan ID="+id+"?")) {
					...
					document.getElementById('form'+id).submit();
    					...
    					document.getElementById("row"+id).style.color = "white";
    					document.getElementById("row"+id).style.backgroundColor = "black";
				}
			}
		}
	</script>
