  ...
  <div class="container-fluid">
      <div class="row">
		...
		<?php if ($this->session->userdata['admin']['status'] == "konten") { ?>
              		<li class="nav-item">
                		<a class="nav-link active" href="<?php echo site_url('admin_Konten/addClient'); ?>">
          ...
          <div class="main-content-container container-fluid px-4">
            ...
            <div class="page-header row no-gutters py-4">
              <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
                ...
                <h3 class="page-title">List Klien</h3>
              </div>
            </div>
            <!-- End Page Header -->
            <!-- Default Light Table -->
              ...
              <div class="col">
                <div class="card card-small mb-4">
                  <div class="card-header border-bottom bg-white" >
                    <h6 class="m-0">Klien yang telah bekerja sama dengan Antara Digital</h6>
                  </div>
				...
				<?php } ?>
                      ...
                      <tbody>
			<?php if (count($this->session->userdata('allKlienData')['id_klien']) == 0) { ?>
				<div style="color: red;">
					Belum ada klien yang terdaftar bekerja sama.
				</div>
			<?php } else { 
				for ($i = 1; $i <= count($this->session->userdata('allKlienData')['id_klien']); $i++) { 								//iterasi untuk baris (x)
					$id = $this->session->userdata('allKlienData')['id_klien'][$i]; ?>
						...
						<?php for ($k = 1; $k <= count($this->session->userdata('columnName')); $k++) {								//iteration for checking the data is it contained "post" or not
							...
							//if ($this->session->userdata('allKlienData')[$columnName][$i] == 'post') {							//if it is containing "post", let the checking proses continued and "allNewsData" is ready to write into page
								for ($j = 1; $j <= count($this->session->userdata('columnName')); $j++) { 						//iterasi untuk kolom (y) pada baris (x) --> [x,y]
									...
									<td><?php echo $this->session->userdata('allKlienData')[$columnName][$i]; ?></td> 
								<?php } ?>
								<form method="post" id="form<?php echo $id ?>" action="<?php echo site_url('admin_Konten/deleteClient/'.$id); ?>">
									<td>
										<input type="hidden" name="abcd" id="abcd" value="def">
										<a href="#" onclick="reassuringDelete('<?php echo $id ?>')">Hapus</a>
									</td>
								</form> 
								<?php break;							
				...
				<tr>
					...
					<form action="<?php echo site_url('admin_Konten/addClient'); ?>" id="form_addClient" method="post">
                          			<td><input name="namaKlien" type="text" placeholder="Nama Klien"></td>
	...
	<script>
		function reassuringDelete(id) {
    			document.getElementById("row"+id).style.color = "white";
    			document.getElementById("row"+id).style.backgroundColor = "red";
			setTimeout(popUp, 10);
			function popUp() {
				if (confirm("Apakah anda yakin ingin menghapus Client dengan ID="+id+"?")) {
					alert("Client dengan ID="+id+" telah berhasil dihapus!");
