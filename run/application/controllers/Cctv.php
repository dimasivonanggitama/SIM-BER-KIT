...
class Cctv extends Goodsyst_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model('ap_cctv_m');
        $this->load->model('ap_cctv_cek_m');
        $this->load->model('ap_cctv_kegi_m');
        $this->load->model('ap_cctv_ref_cek_m');
    }
    ...
    public function index($bulan = NULL, $tahun = NULL)
    {
        ...
        if ($bulan == NULL && $tahun == NULL) {
            $this->db->join('ap_admin', 'ap_admin.id_admin=ap_cctv.id_atasan', 'left');
            $this->db->order_by('ap_cctv.id_cctv DESC');
            $this->data['content'] = $this->ap_cctv_m->get_by(array(
                'ap_cctv.id_admin' => $this->session->userdata('id')
            ));
    ...
    public function edit($id = NULL)
    {
        ...
        $this->data['atasan'] = $this->ap_admin_m->get();
        if ($id) {
            $this->data['content'] = $this->ap_cctv_m->get($id);
            ...
            $data = array(
                ...
                'alamat_cctv' => $this->input->post('alamat'),
            ...
            $this->data['content'] = $this->ap_cctv_m->get_new();
            $data = array(
                'id_cctv' => $this->ap_cctv_m->increment(),
                'bulan_cctv' => $this->input->post('bulan'),
                'tahun_cctv' => $this->input->post('tahun'),
                'id_perusahaan' => $this->input->post('kawasan'),
                'alamat_cctv' => $this->input->post('alamat'),
        ...
        $rules = $this->ap_cctv_m->rules;
        $this->form_validation->set_rules($rules);
        
        if ($this->form_validation->run() == TRUE) {
            
            $this->ap_cctv_m->save($data, $id);
            if ($id == NULL) {
                $userx = array();
                array_push($userx, $this->input->post('atasan'));
                $this->notifikasi('cctv_spv', 'Laporan Hanggar CCTV', $userx);
            }
    ...
    public function cek($id = NULL)
    {
        ...
        $tampung_ref=$this->ap_cctv_ref_cek_m->get();
        $tampung_cek=$this->ap_cctv_cek_m->get_by(array('id_cctv' => $id));
        if ($tampung_cek == NULL){
            // $msg='test';
            // echo '<script type="text/javascript">alert("' . $msg . '")</script>';
            foreach($tampung_ref as $val){
                $data = array(
                    'id_cctv_cek' => $this->ap_cctv_cek_m->increment(),
                    'id_cctv' => $id,
                    'desc_cctv_cek' => $val->id_cctv_ref,
                    'ya_tidak_cctv_cek' => NULL,
                    'ket_cctv_cek' => NULL
                );
                $this->ap_cctv_cek_m->save($data);
            }
        }

        $this->data['cctv'] = $this->ap_cctv_m->get($id);

        $this->data['cctv_ref'] = $this->ap_cctv_ref_cek_m->get();
        
        $this->data['cctv_cek'] = $this->ap_cctv_cek_m->get_by(array(
            'id_cctv' => $id
        ));
        
        $sql = "SELECT * FROM ap_cctv_cek WHERE id_cctv=? ORDER BY id_cctv_cek DESC LIMIT 1";
        ...
        $this->data['cctv_kegi'] = $this->ap_cctv_kegi_m->get_by(array(
            'id_cctv' => $id
        ));
        ...
        if ($id) {
            $this->data['content'] = $this->ap_cctv_cek_m->get_new();
            $data = array(
                'id_cctv_cek' => $this->ap_cctv_cek_m->increment(),
                'id_cctv' => $id,
        ...
        if ($id) {
            ...
            $this->data['content_kegi'] = $this->ap_cctv_kegi_m->get_new();
            
            $data_kegi = array(
                'id_cctv_kegiatan' => $this->ap_cctv_kegi_m->increment(),
                'id_cctv' => $id,
        ...
        if ($this->input->post('cek') != NULL) {
            $rules = $this->ap_cctv_cek_m->rules;
            $this->form_validation->set_rules($rules);
        ...
        if ($this->input->post('kegiatan') != NULL) {
            $rules = $this->ap_cctv_kegi_m->rules;
            $this->form_validation->set_rules($rules);
            
            if ($this->form_validation->run() == TRUE) {
                $this->ap_cctv_kegi_m->save($data_kegi);
                redirect($this->uri->rsegment(1) . '/cek/' . $id);
            }
        }
    ...
    {
        $this->db->delete('ap_cctv_cek', array(
            'id_cctv' => $id
        ));
        $this->db->delete('ap_cctv_kegiatan', array(
            'id_cctv' => $id
        ));
        $this->ap_cctv_m->delete($id);
        redirect($this->uri->rsegment(1) . '/index/');
    }

    public function delete_cek($id, $parent)
    {
        $this->ap_cctv_cek_m->delete($id);
        redirect($this->uri->rsegment(1) . '/cek/' . $parent);
    }

    public function delete_kegi($id, $parent)
    {
        $this->ap_cctv_kegi_m->delete($id);
        redirect($this->uri->rsegment(1) . '/cek/' . $parent);
    }
    ...
    public function unduh_pdf_cctv($id)
    {
        ...
        $datatbl=''; 
        $datatbl=$this->load->view('cctv/view_download', $this->data, true);;
    ...
    public function unduh_pdf_cctv_old($id)
    {
        ...
        $datatbl=''; 
        $datatbl=$this->load->view('cctv/view_download_old', $this->data, true);;
    ...
    public function ajax_setuju_semua()
    {
        ...
        for ($x = 0; $x < count($tableData); $x++) {
            ...
            $this->db->update('ap_cctv_cek', $data);
        }
    }
    ...
    public function ajax_setuju()
    {
        ...
        $this->data['content'] = $this->ap_cctv_cek_m->get($id);
        $data = array(
            'ya_tidak_cctv_cek' => 1,
            'ket_cctv_cek' => $catatan
        );
        $this->ap_cctv_cek_m->updateM($data, $id_cctv, $id_cek);
    }
    ...
    public function ajax_batal_setuju()
    {
        ...
        $tampung_cek_bawah=$this->ap_cctv_cek_m->get_by(array('id_cctv' => $id_cctv, 'id_cctv_cek >=' => $id_cek));
        if ($tampung_cek_bawah != NULL){
            ...
            foreach($tampung_cek_bawah as $val){
                ...
                $this->ap_cctv_cek_m->updateM($data, $val->id_cctv, $val->id_cctv_cek);
            }
        }
    }
    ...
    public function ajax_tidak_setuju()
    {
        ...
        $tampung_cek_atas=$this->ap_cctv_cek_m->get_by(array('id_cctv' => $id_cctv, 'id_cctv_cek <' => $id_cek,'ya_tidak_cctv_cek' => NULL));
        ...
        $tampung_cek_bawah=$this->ap_cctv_cek_m->get_by(array('id_cctv' => $id_cctv, 'id_cctv_cek >' => $id_cek,'ya_tidak_cctv_cek' => NULL));
        if ($tampung_cek_bawah != NULL){
            // $msg='test';
            // echo '<script type="text/javascript">alert("' . $msg . '")</script>';
            foreach($tampung_cek_bawah as $val){
