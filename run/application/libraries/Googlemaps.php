...
class Googlemaps {
	...
	function create_map()
	{
		...
		$this->output_html = '';
		$this->data['apiKey']='AIzaSyD_6ur2WG0DbSHeueQ-QzKkfl2AsWMyV3k';
		
		if ($this->maps_loaded == 0)
		{
			if ($this->apiKey!="AIzaSyD_6ur2WG0DbSHeueQ-QzKkfl2AsWMyV3k") 
			{
				if ($this->https) { $apiLocation = 'https'; }else{ $apiLocation = 'https'; }
				$apiLocation .= '://maps.googleapis.com/maps/api/js?key='.$this->data['apiKey'].'&callback=initMap';
			}
			else
			{
			...
			<script type="text/javascript" src="' . ( ($this->https) ? 'https' : 'https' ) . '://developers.google.com/maps/documentation/javascript/examples/markerclusterer/markerclusterer.js"></script>
			'; }
		}
		if ($this->jsfile=="") {
			$this->output_js .= '
